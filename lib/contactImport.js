...
function phoneInputText(value) {
  if (typeof value === 'number') {
    // Excel stores numeric cells as JavaScript numbers. Only use values that
    // remain exact when represented as a number; never manufacture digits.
    if (!Number.isFinite(value) || !Number.isSafeInteger(value) || value <= 0 || /e/i.test(String(value))) {
      throw new Error('Could not safely interpret this phone number.')
    }
    return String(value)
  }
...
function normalizePhone(value, { country = 'ZM' } = {}) {
  const raw = phoneInputText(value)
...
function originalPhoneValue(value) {
  try { return phoneInputText(value) } catch { return cleanText(value, 80) }
}
...
function buildImportPlan({ headers, rows, mapping, existingContacts, country = 'ZM' }) {
  ...
  rows.forEach((rawRow, index) => {
    ...
    const originalPhone = originalPhoneValue(phoneRaw)
    ...
    if (!reason && seen.has(phone)) {
      duplicateRows += 1
      if (preview.length < 20) preview.push({ row_number: rowNumber, status: 'duplicate', name, original_phone: originalPhone, zedping_number: phone, email, reason: 'Duplicate in file' })
      return
    }
    if (reason) {
      invalidRows += 1
      const status = /safely interpret/i.test(reason) ? 'uninterpretable' : 'invalid'
      if (preview.length < 20) preview.push({ row_number: rowNumber, status, name, original_phone: originalPhone, zedping_number: '', email, reason })
      return
    }
    seen.add(phone)
    ...
    if (preview.length < 20) preview.push({ row_number: rowNumber, status: existing ? 'existing' : 'ready', name, original_phone: originalPhone, zedping_number: phone, email, reason: '' })
  })
  ...
  return {
    ...
    summary: {
      ...
      ready_to_import: newCount,
      new_contacts: newCount,
      existing_contacts: existingCount,
      invalid_rows: invalidRows,
      duplicate_rows: duplicateRows,
      skipped_rows: invalidRows + duplicateRows
    }
  }
}
...
module.exports = { MAX_ROWS, normalizePhone, phoneInputText, readMapping, buildImportPlan, safeFilename }
