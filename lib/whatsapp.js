...
const sendTemplateMessage = async (phoneNumberId, to, template, accessToken) => {
  const components = template.headerMedia ? [{ type: 'header', parameters: [{ type: template.headerMedia.type, [template.headerMedia.type]: { id: template.headerMedia.id } }] }] : undefined
