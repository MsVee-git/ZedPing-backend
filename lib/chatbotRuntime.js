...
function simulateFlow(definition, inputs = [], { contentItems = new Map() } = {}) {
  const valid = validateDefinition(definition, { contentItems })
  ...
  let waitingStepId = null

  const say = (text) => {
    output.push(text)
    transcript.push({ speaker: 'zedping', text })
  }
  const nextInput = () => {
    if (inputIndex >= suppliedInputs.length) return { present: false }
    const value = String(suppliedInputs[inputIndex++] ?? '')
    transcript.push({ speaker: 'you', text: value })
    return { present: true, value }
  }
  const waiting = () => ({ output, transcript, captured, terminal: null, waiting_for_input: true })

  for (let guard = 0; guard < 100 && current; guard += 1) {
    ...
    if (step.type === 'content') {
      // Content remains owned by Content Library. The simulator deliberately
      // identifies the share without copying workspace content into its result.
      say('[Content Library item]')
      current = step.next_step_id
      waitingStepId = null
      continue
    }
    if (step.type === 'ask_capture') {
      if (waitingStepId !== step.id) {
        say(step.text)
        waitingStepId = step.id
      }
      const input = nextInput()
      // Missing test input means a real customer has not replied yet. It is
      // neither an invalid answer nor a validation attempt.
      if (!input.present) return waiting()
      const result = validateCapture(input.value, step.capture)
      if (!result.ok) {
        const attempts = Number(attemptsByStep.get(step.id) || 0) + 1
        attemptsByStep.set(step.id, attempts)
        if (attempts >= 2) return { output, transcript, captured, terminal: step.capture.failure_action, waiting_for_input: false }
        // This mirrors the live retry message and keeps the step waiting for
        // the next supplied customer answer.
        say(`${result.reason}. ${step.text}`)
        continue
      }
      captured[step.capture.key] = result.value
      attemptsByStep.delete(step.id)
      current = step.next_step_id
      waitingStepId = null
      continue
    }
    if (step.type === 'choose_option') {
      if (waitingStepId !== step.id) {
        say(formatChoicePrompt(step))
        waitingStepId = step.id
      }
      const input = nextInput()
      if (!input.present) return waiting()
      const selected = findChoice(step, input.value)
      if (!selected) {
        const attempts = Number(attemptsByStep.get(step.id) || 0) + 1
        attemptsByStep.set(step.id, attempts)
        if (attempts >= 2) return { output, transcript, captured, terminal: 'human_handoff', waiting_for_input: false }
        say(`Please reply with one of the numbered choices.\n${formatChoicePrompt(step)}`)
        continue
      }
      attemptsByStep.delete(step.id)
      if (selected.outcome) return { output, transcript, captured, terminal: selected.outcome, waiting_for_input: false }
      current = selected.next_step_id
      waitingStepId = null
      continue
    }
    if (step.type === 'human_handoff') return { output, transcript, captured, terminal: 'human_handoff', waiting_for_input: false }
    if (step.type === 'end') {
      if (step.text) say(step.text)
      return { output, transcript, captured, terminal: 'end', waiting_for_input: false }
    }
  }
  return { output, transcript, captured, terminal: 'failed', waiting_for_input: false }
}
